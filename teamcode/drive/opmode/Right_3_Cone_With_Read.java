...
public class Right_3_Cone_With_Read extends LinearOpMode {
    ...
    protected void runSample() throws InterruptedException {
        ...
        TrajectorySequence traj1 = drive.trajectorySequenceBuilder(startPose)
                ...
                .splineToSplineHeading(new Pose2d(-28.75, 38, Math.toRadians(0)), Math.toRadians(0))
                .build();

        TrajectorySequence traj2 = drive.trajectorySequenceBuilder(traj1.end())
                .setTangent(Math.toRadians(45))
                .splineToSplineHeading(new Pose2d(-26.5, 5.5, Math.toRadians(135)), Math.toRadians(135))
                .build();
        //Drop cone #1
        ...
        TrajectorySequence traj3 = drive.trajectorySequenceBuilder(traj2.end())
                .setTangent(Math.toRadians(45))
                .splineToSplineHeading(new Pose2d(-63, 7.5, Math.toRadians(-180)), Math.toRadians(180))
                .build();
        //To pole drop cone #2
        TrajectorySequence traj4 = drive.trajectorySequenceBuilder(traj3.end())
                .setTangent(Math.toRadians(180))
                .splineToSplineHeading(new Pose2d(-27, 1.5, Math.toRadians(135)), Math.toRadians(135))
                .build();
        //To cone stack for 3rd cone
        TrajectorySequence traj5 = drive.trajectorySequenceBuilder(traj4.end())
                .setTangent(Math.toRadians(45))
                .splineToSplineHeading(new Pose2d(-63, 6.5, Math.toRadians(-180)), Math.toRadians(-180))
                .build();
        // To pole to drop cone 3
        TrajectorySequence traj6 = drive.trajectorySequenceBuilder(traj5.end())
                .setTangent(Math.toRadians(180))
                .splineToSplineHeading(new Pose2d(-27.5, -.5, Math.toRadians(135)), Math.toRadians(135))
                .build();
...
// Start Park trajectories
        ...
        while (!(Parkacola > 0)) {
            ...
            if (colors.blue > .55 && colors.blue < .85) {
                Parkacola = 1;
                telemetry.addData("Color: ", "Black==park#1");
                telemetry.addData("Parkacola", Parkacola);
                telemetry.update();
                Read = true;
            } else if (colors.red > .42 && colors.red < .5  && colors.blue > .85) {
                Parkacola = 2;
                telemetry.addData("Color: ", "Blue==Park#2");
                telemetry.addData("Parkacola", Parkacola);
                telemetry.update();
                Read = true;
            } else if (colors.red > .575 && colors.blue <.85) {
                Parkacola = 3;
